...
public class BasicCommands {
	...
	public static void setAllyOrEnemy(Unit unit) {			// we need it to identify ally or enemy
		
		if(unit.getId()<20 || unit.getId() == 99) {
			unit.setGroup(0);
		}
		
		if((unit.getId() > 19 && unit.getId() <40) || unit.getId() == 100) {
			unit.setGroup(1);
		}
	...
	public static void setUnitAttack(ActorRef out, Unit unit, int attack) {
		try {
			unit.setAttack(attack);		// we need it
			ObjectNode returnMessage = Json.newObject();
	...
	public static void setUnitHealth(ActorRef out, Unit unit, int health) {
		try {
			unit.setHealth(health);		//we need it
			unit.setMaxHealth(health);	// initial health/max health
			ObjectNode returnMessage = Json.newObject();
